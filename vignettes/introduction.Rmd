...
- templates with zero SNVs (`total_disruption_count = 0`) override to bin 5 (Dirt).
...
- `balanced`:
  ...
  2. `disruption_bin`
  3. `relative_crispr_mfh` (Ascending)
  4. `total_snv_quality_score`
  5. `n_snvs`
  6. `is_ag_risky`
...
- `safety_first`:
  ...
  3. `disruption_bin`
  4. `relative_crispr_mfh` (Ascending)
  5. `n_snvs`
