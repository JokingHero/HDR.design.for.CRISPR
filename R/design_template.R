...
# exon_bp = 3
# probes = TRUE
# primer3 = "/home/ai/Soft/primer3-2.6.1/src/primer3_core" # or default ""
# one_for_all = FALSE
# source("./R/utils.R")
...
#' @title Main function for the design of the templates.
#' @param one_for_all One template for all of the guides.
#'
design_template <- function(one_for_all = FALSE) {
  function(
    ...
    if (one_for_all) { # one template for all the guides
      ...
      if (is.null(muts)) {
        stop("Could not design synonymous SNPs for this loci :(")
      }
      mutated_seq <- replaceAt(genomic_seq[[1]],
                               at = ranges(muts[[1]]),
                               value = DNAStringSet(muts[[1]]$replacement))
      repair_template <- GRanges(seqnames = mutation_name,
      ...
      names(repair_template) <- paste0("Template_",
                                       "Mut_", paste0(names(muts[[1]]), collapse = "_"))
      ...
      if (probes) {
        ...
        hdr_probes <- select_probes(c(muts[[1]], origin_mutation),
...
#' @export
#'
design_one_template_for_each_guide <- design_template(one_for_all = FALSE)


#' @title Design best CRISPR guide for fixing mutation
#'
#' @description Designs one optimized template for all of the guides overlapping site of interest.
#' Templates are optimized, by selecting synonymous SNPs that have the highest chance of disrupting
#' guides, minimizing disruption to other genomic functions by checking with submitted annotation file,
#' prioritizing SNPs that are already available in the population submitted through `snps
#' parameter. Guides are ranked first, by their inhibition potential, then by the quality of their snps,
#' and further by efficiency scoring. The difference here to the `design_one_template_for_each_guide`
...
#' @inheritParams design_template
#' @inherit design_template params return
#' @export
#'
design_one_template_for_all_guides <- design_template(one_for_all = TRUE)
